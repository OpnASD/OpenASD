...
.PHONY: all boot kernel userland uki live-image prepare-run run run-debug run-headless-debug run-gui-debug bfd bfd-debug bfd-autotest iso clean install install-fresh
...
userland/fastfetch/build/fastfetch: kernel/arch/syscall.c kernel/arch/elf.c userland/fastfetch/fastfetch.c
	$(MAKE) -C userland/fastfetch
...
live-image: all
	...
	@for b in ls cat mkdir rm touch echo pwd sysinfo uname uptime id whoami kill hexdump wc; do \
		mcopy -i "$(LIVE_IMG)" userland/bin/build/$$b ::/bin/$$b; \
	done
	@mcopy -i "$(LIVE_IMG)" userland/fastfetch/build/fastfetch ::/bin/fastfetch
	@mcopy -i "$(LIVE_IMG)" userland/asded/build/asded         ::/bin/asded
...
install: all
	$(call check-mtools)
	@mkdir -p "$(DISK_EFI_DIR)" "$(DISK_BOOT_DIR)"
	...
	@mcopy -i "$(DISK_IMG)" "$(DISK_EFI_DIR)/BOOTX64.EFI" ::/EFI/BOOT/BOOTX64.EFI
	@mcopy -i "$(DISK_IMG)" "$(DISK_EFI_DIR)/asdkernel.bin" ::/EFI/BOOT/asdkernel.bin
	@mcopy -i "$(DISK_IMG)" "$(DISK_EFI_DIR)/asdboot.conf" ::/EFI/BOOT/asdboot.conf
	@mcopy -i "$(DISK_IMG)" "$(DISK_BOOT_DIR)/asdkernel.bin" ::/boot/asdkernel.bin
	@mcopy -i "$(DISK_IMG)" "$(DISK_BOOT_DIR)/asdboot.conf" ::/boot/asdboot.conf
	@mcopy -i "$(DISK_IMG)" userland/sh/build/asdsh ::/bin/asdsh
	@for b in ls cat mkdir rm touch echo pwd sysinfo uname uptime id whoami kill hexdump wc; do \
		mcopy -i "$(DISK_IMG)" userland/bin/build/$$b ::/bin/$$b; \
	done
	@mcopy -i "$(DISK_IMG)" userland/fastfetch/build/fastfetch ::/bin/fastfetch
	@mcopy -i "$(DISK_IMG)" userland/asded/build/asded         ::/bin/asded
	@mcopy -i "$(DISK_IMG)" userland/sbin/build/asdlog          ::/sbin/asdlog
	@mcopy -i "$(DISK_IMG)" userland/sbin/build/netd            ::/sbin/netd
	@printf '#ASDPW1\n' > "$(RUN_DIR)/asdpw1.stub"
...
# Same as bfd, but logs kernel serial and QEMU errors for debugging.
bfd-debug: all
	@mkdir -p "$(DEBUG_DIR)"
...
# Writes $(SERIAL_LOG); exits after ~25s (timeout) or when QEMU is killed.
...
bfd-autotest: all
	...
	@if grep -q '\*\*\* EXCEPTION' "$(SERIAL_LOG)" 2>/dev/null; then \
		echo "FAIL: kernel exception in $(SERIAL_LOG)"; exit 1; \
	elif grep -q '\[autotest\] OK' "$(SERIAL_LOG)" 2>/dev/null; then \
		echo "PASS: fastfetch autotest completed"; \
	else \
		echo "WARN: no [autotest] OK (timeout? check $(SERIAL_LOG))"; exit 1; \
	fi
