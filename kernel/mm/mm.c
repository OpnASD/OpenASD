...
void amm_vmap_destroy(vmap_t *map) {
    /* Unmap and free all user regions */
    vregion_t *r = map->head;
    while (r) {
        vregion_t *next = r->next;
        /* Free physical pages backing this region */
        for (vaddr_t va = r->base; va < r->base + r->size; va += PAGE_SIZE) {
            uint64_t *pml4 = phys_to_virt(map->cr3);
            ...
            e = pt[PT_IDX(va)];
            if (e & PTE_PRESENT)
                amm_phys_free(e & PTE_ADDR_MASK, 0);
        }
        r = next;
    }
    /* Free PML4 (not the kernel half entries themselves) */
