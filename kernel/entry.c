...
static void seed_asdsh_into_ramfs(void) {
    extern int ramfs_root_mkdir(const char *path);
    extern int ramfs_root_writefile(const char *path, const void *data, size_t size);

    const char *bins[] = { "asdsh", "fastfetch", NULL };
    for (int i = 0; bins[i]; i++) {
        ...
        if (vfs_stat(dst_path, &st) == 0)
            continue;

        if (vfs_stat(src_path, &st) != 0) continue;

        fd_t fd = vfs_open(src_path, VFS_O_READ, NULL);
        if (fd <= 0) continue;

        static uint8_t buf[512 * 1024];
        size_t total = 0, got = 0;
        while (total < sizeof(buf)) {
            if (vfs_read(fd, buf + total, sizeof(buf) - total, &got) != 0 || got == 0)
                break;
            total += got;
        }
        vfs_close(fd);
        ...
        if (ramfs_root_writefile(dst_path, buf, total) == 0) {
            serial_puts("    seeded ");
            serial_puts(dst_path);
            serial_puts(" from disk image\n");
        }
    }
}
    ...
    /* Enable SSE/AVX for userland (fastfetch uses it for CPUID/xgetbv) */
    {
        ...
        if (edx & (1 << 25)) { /* SSE bit */
            ...
            if (ecx & (1 << 26)) { /* XSAVE bit */
                ...
                /* Enable AVX in XCR0 if OSXSAVE is set */
                __asm__ volatile(
                    "xor %%ecx, %%ecx\n\t"
                    "xgetbv\n\t"
                    "or $7, %%eax\n\t" /* Set X87, SSE, AVX bits */
                    "xsetbv"
                    : : : "rax", "rcx", "rdx"
                );
...
void kernel_main_body(asd_bib_t *bib, uint64_t magic) {
    ...
    if (block_count() == 1) {
        ...
        if (d0) {
            ...
            ramfs_mount_root();
            ramfs_root_mkdir("/bin");
            serial_puts("    vfs: ramfs at / (installed system)\n");

            int disk_mounted = 0;
            uint8_t hdr_sec[512];
            if (d0->read(d0, 1, 1, hdr_sec) == 0 &&
                memcmp(hdr_sec, "EFI PART", 8) == 0) {
                uint8_t part_sec[512];
                if (d0->read(d0, 2, 1, part_sec) == 0) {
                    for (int pi = 0; pi < 2 && !disk_mounted; pi++) {
                        const uint8_t *pe = part_sec + (pi * 128);
                        uint64_t first = rd64le(pe + 32);
                        uint64_t last  = rd64le(pe + 40);
                        if (first == 0 || last < first)
                            continue;
                        serial_puts("    gpt: mount partition ");
                        serial_putu((uint64_t)(pi + 1));
                        serial_puts(" at /boot (LBA ");
                        serial_putu(first);
                        serial_puts(")\n");
                        if (try_mount_fat_volume(d0, first, (last - first) + 1, "/boot")) {
                            stat_info_t st;
            ...
            } else if (try_mount_fat_volume(d0, 0, d0->sector_count, "/boot")) {
                stat_info_t st;
                if (vfs_stat("/boot/bin/asdsh", &st) == 0 ||
                    vfs_stat("/boot/asdboot.conf", &st) == 0 ||
                    vfs_stat("/boot/boot/asdboot.conf", &st) == 0)
    ...
    /* After /root is set up — ramfs_root_init must not wipe the / ramfs. */
    seed_asdsh_into_ramfs();
