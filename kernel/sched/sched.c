...
static void rb_remove_node(rb_root_t *root, rb_node_t *z) {
    /* Standard red-black delete. Abbreviated for clarity. */
    rb_node_t *y = z, *x = NULL, *x_parent = NULL;
    int orig_color = y->color;

    if (!z->left) {
        x = z->right; x_parent = z->parent;
        /* transplant z with z->right */
        if (!z->parent) root->root = z->right;
    ...
    /* Fixup omitted for brevity — full implementation in sched_rbtree.c */
    (void)orig_color; (void)x; (void)x_parent;
