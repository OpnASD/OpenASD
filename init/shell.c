 ...
 *  - External binary execution from /bin via elf_spawn.
...
static void cmd_ls(const char *arg) {
    ...
    if (path_is_root(path)) {
        for (int i = 0; g_mounts[i]; i++) {
            serial_puts("d  ");
            serial_puts(g_mounts[i] + 1);
            serial_puts("/\n");
        }
        return;
    }
    ...
    uint32_t n = 0;
    if (vfs_readdir(path, ents, 256, &n) != 0) {
...
static void cmd_cd(const char *arg) {
    ...
    char path[VFS_PATH_MAX];
    resolve_path(arg, path);
    if (path_is_root(path)) { strncpy(g_cwd, "/", VFS_PATH_MAX); return; }
...
static void try_exec(const char *cmd) {
    ...
    if (cmd[0] == '/') {
        strncpy(binpath, cmd, VFS_PATH_MAX);
        binpath[VFS_PATH_MAX - 1] = '\0';
    } else {
        strncpy(binpath, "/bin/", VFS_PATH_MAX);
        size_t n = strlen(binpath);
        /* Extract just the command name (first token) */
        size_t i = 0;
        while (cmd[i] && cmd[i] != ' ' && n + i < (size_t)(VFS_PATH_MAX - 1)) {
            binpath[n + i] = cmd[i]; i++;
        }
        binpath[n + i] = '\0';
    }
    ...
    if (vfs_stat(binpath, &st) != 0) {
        serial_puts("command not found: "); serial_puts(cmd); serial_puts("\n");
        return;
    }

    /* Build argv from the command line */
    ...
    argv[argc] = NULL;
    /* Replace first arg with full path */
    argv[0] = binpath;
    ...
    uint32_t pid = elf_spawn(binpath, argv, NULL);
    if (!pid) {
    ...
    exit_info_t ei;
    if (sched_reap((pid_t)pid, &ei) != 0)
        serial_puts("exec: wait failed\n");
    else
        serial_puts("[exec] done\n");
}
