...
/* ------------------------------------------------------------------ */
/* FIX Bug 7: Deploy base directory structure to installed disk        */
/* ------------------------------------------------------------------ */
/*
 * After the FAT image is copied to the ESP, we mount it via the VFS
 * and create the standard OpenASD directory tree:
 *
 *   /kernel  -- kernel image and modules
 *   /bin     -- user-space binaries
 *   /sbin    -- system daemons
 *   /boot    -- bootloader and boot config
 *   /etc     -- system configuration
 *   /var     -- variable data (logs, run-time state)
 *   /var/log -- log files
 *   /home    -- user home directories
 *   /root    -- root home directory
 *   /tmp     -- temporary files
 *
 * The directories are created inside the VFS which is backed by the
 * live FAT image already in memory; the image will be written to the
 * target ESP by install_boot_media() afterwards.
 */
static void deploy_directory_structure(const char *user_name) {
    serial_puts("  [deploy] Creating base directory structure\n");
    ...
    /* Write /etc/hostname */
    {
        fd_t fd = vfs_open("/etc/hostname",
                           VFS_O_WRITE | VFS_O_CREAT | VFS_O_TRUNC,
                           NULL);
        if (fd > 0) {
    ...
    /* Write /etc/os-release */
    {
        ...
        if (fd > 0) {
            static const char *release =
                ...
                "VERSION=\"0.1\"\n"
                "ID=openasd\n"
                "PRETTY_NAME=\"OpenASD 0.1\"\n"
                "HOME_URL=\"https://github.com/openasd\"\n";
            vfs_write(fd, release, strlen(release));
            vfs_close(fd);
    ...
    /* Write /etc/motd */
    {
        ...
        if (fd > 0) {
            static const char *motd =
                "Welcome to OpenASD 0.1\n"
                "Type 'fastfetch' for system info, 'help' for commands.\n";
...
int installer_run(void) {
    ...
    /* ---- Step 3: Directory layout on live image (ramfs paths) ---- */
    serial_port_puts("[installer] preparing directory layout...\n");
    tui_screen_progress(2, 0, 1);
